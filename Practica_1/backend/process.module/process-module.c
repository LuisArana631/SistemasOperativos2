...
static int my_proc_show(struct seq_file *m, void *v)
{
        ...
        for_each_process(task) {
            ...
            if (task->mm) {
                ...
                seq_printf(m, "{\"name\": \"%s\", \"pid\":%d, \"state\":%lu, \"father\":%d, \"usedCpu\": \"%d\", \"usedRAM\": \"%lu\", \"codeSize\": \"%lu\"},\n",task->comm , task->pid, task->state, task->parent->pid, task->recent_used_cpu, (rss/100000)*100/978 , task->mm->end_code - task->mm->start_code);
            }else{
            ...
            list_for_each(list, &task->children){
                ...
                if (task_child->mm) {
                    ...
                    seq_printf(m, "{\"name\": \"%s\", \"pid\":%d, \"state\":%lu, \"father\":%d, \"usedCpu\": \"%d\", \"usedRAM\": \"%lu\", \"codeSize\": \"%lu\"},\n",task_child->comm , task_child->pid, task_child->state, task_child->parent->pid, task_child->recent_used_cpu, (rss/100000)*100/978, task->mm->end_code - task->mm->start_code);
                }else{
